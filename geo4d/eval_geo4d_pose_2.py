"""
Evaluate Geo4D camera pose estimation on ray_mot eval outputs.

Reads gt_rgb.mp4 from each sample dir (produced by inference_single_gpu_ray_mot_eval.py),
runs Geo4D to get predicted camera extrinsics, then compares against GT cameras saved in
gt_cameras.npz (c2w, shape T x 4 x 4).

Metrics: AUC3 and AUC30 (same as Table 2 in the DA3 paper).
...
def main() -> None:
    ...
    for sample_dir in tqdm(sample_dirs, desc="Geo4D pose eval"):
        ...
        pred_w2c = np.linalg.inv(pred_c2w)  # (T, 4, 4)
