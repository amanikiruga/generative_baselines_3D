...
ROOT = Path("/net/holy-isilon/ifs/rc_labs/ydu_lab/Lab/akiruga/generative_baselines/eval_outputs_v4_n50")
...
def main():
    methods_nvs   = ["ours_final_nvs_eval", "ours_nvs_nvs_eval",
                     "wan_flf_nvs", "wan_flf_nvs_text",
                     "dfot_nvs", "seva_nvs"]
    methods_pose  = ["ours_final_pose_eval", "ours_nvs_pose_eval",
                     "geo4d_pose", "raydiffusion_pose"]
    methods_depth      = ["ours_final_depth_eval", "ours_nvs_depth_eval",
                          "geo4d_depth", "chronodepth_depth"]
    methods_depth_disp = ["ours_final_depth_disp_eval", "ours_nvs_depth_disp_eval",
                          "geo4d_depth_disp", "chronodepth_depth_disp"]
    datasets_nvs   = ["re10k", "dl3dv", "dl3dv_test", "tanksandtemples", "scannetpp", "vkitti2", "aria", "spatialvid_nvs", "agibot_world", "7scenes", "tum"]
    datasets_pose  = ["re10k", "dl3dv", "dl3dv_test", "tanksandtemples", "scannetpp", "vkitti2", "aria", "spatialvid_nvs", "agibot_world", "7scenes", "tum", "sintel", "eth3d", "dtu"]
    datasets_depth = ["scenenet_depth", "vkitti2", "aria", "dl3dv", "scannetpp", "7scenes", "tum", "bonn", "kitti", "sintel", "eth3d", "dtu"]

    name_map = {
        "ours_final_nvs_eval": "Ours (final)",
        "ours_nvs_nvs_eval": "Ours (NVS)",
        "wan_flf_nvs": "Wan 2.1 FLF",
        "wan_flf_nvs_text": "Wan 2.1 FLF (text)",
        "dfot_nvs": "DFoT", "seva_nvs": "SEVA",
        "ours_final_pose_eval": "Ours (final)",
        "ours_nvs_pose_eval": "Ours (NVS)",
        "geo4d_pose": "GEO4D", "raydiffusion_pose": "RayDiffusion",
        "ours_final_depth_eval": "Ours (final)",
        "ours_nvs_depth_eval": "Ours (NVS)",
        "geo4d_depth": "GEO4D", "chronodepth_depth": "ChronoDepth",
        "ours_final_depth_disp_eval": "Ours (final)",
        "ours_nvs_depth_disp_eval": "Ours (NVS)",
        "geo4d_depth_disp": "GEO4D",
        "chronodepth_depth_disp": "ChronoDepth",
    }

    lines = []
    lines.append("# Results V4 (n=10/50) — 3 ckpts × baselines, metadata-pinned video set (metadata_v3_n50.json)")
    lines.append("# OURS_FINAL = 2026-05-04/12-34-53 ; OURS_NVS = 2026-04-29/14-31-15")
    lines.append("")
    ...
    for m in methods_pose:
        ...
        lines.append("| " + " | ".join(row) + " |")
    lines.append("")
    lines.append("## Depth — depth-space LAD2 (Abs Rel ↓ / RMSE ↓ / δ<1.25 ↑)")
    lines.append("")
    ...
    out_path = ROOT.parent / "RESULTS_V4_N50.md"
    out_path.write_text("\n".join(lines) + "\n")
