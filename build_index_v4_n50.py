...
EVAL = ROOT / "eval_outputs_v4_n50"
EVAL_REL = "eval_outputs_v4_n50"
...
def pose(dirp):
    """V3: ATE / RPE_trans / RPE_rot from final_stats.json."""
    d = read_json(Path(dirp)/"final_stats.json")
...
def depth(dirp):
    """V3: Abs Rel / RMSE / δ<1.25 from final_stats.json."""
    d = read_json(Path(dirp)/"final_stats.json")
    if not d: return None
    if all(k in d for k in ("abs_rel","rmse","delta_1")):
        return d["abs_rel"], d["rmse"], d["delta_1"]
    return None
...
NVS_METHODS = [
    ("Ground truth",         "ours_final_nvs",    None,                       "gt_rgb.mp4",      "video"),
    ("Ours (final)",         "ours_final_nvs",    "ours_final_nvs_eval",      "pred_rgb.mp4",    "video"),
    ("Ours (NVS)",           "ours_nvs_nvs",      "ours_nvs_nvs_eval",        "pred_rgb.mp4",    "video"),
    ("Wan 2.1 FLF",          "wan_flf_nvs",       "wan_flf_nvs",              "pred_rgb.mp4",    "video"),
    ("Wan 2.1 FLF (text)",   "wan_flf_nvs_text",  "wan_flf_nvs_text",         "pred_rgb.mp4",    "video"),
    ("DFoT (only 8 frames)", "dfot_nvs",          "dfot_nvs",                 "pred_rgb.mp4",    "video"),
    ("SEVA",                 "seva_nvs",          "seva_nvs",                 "samples-rgb.mp4", "video"),
]
POSE_METHODS = [
    ("Ground truth (RGB)", "ours_final_pose_depth",      None,                        "gt_rgb.mp4",            "video"),
    ("Ours (final)",       "ours_final_pose_depth",      "ours_final_pose_eval",      "camera_trajectory.png", "img"),
    ("Ours (NVS)",         "ours_nvs_pose_depth",        "ours_nvs_pose_eval",        "camera_trajectory.png", "img"),
...
DEPTH_METHODS = [
    ("Ground truth (RGB)",   "ours_final_pose_depth", None,                          "gt_rgb.mp4",            "video"),
    ("GT depth",             "ours_final_pose_depth", None,                          "gt_depth.mp4",          "video"),
    ("Ours (final)",         "ours_final_pose_depth", "ours_final_depth_eval",       "pred_depth.mp4",        "video"),
    ("Ours (NVS)",           "ours_nvs_pose_depth",   "ours_nvs_depth_eval",         "pred_depth.mp4",        "video"),
...
DEPTH_DISP_METHODS = [
    ("Ground truth (RGB)",   "ours_final_pose_depth", None,                          "gt_rgb.mp4",            "video"),
    ("GT depth",             "ours_final_pose_depth", None,                          "gt_depth.mp4",          "video"),
    ("Ours (final)",         "ours_final_pose_depth", "ours_final_depth_disp_eval",  "pred_depth.mp4",        "video"),
    ("Ours (NVS)",           "ours_nvs_pose_depth",   "ours_nvs_depth_disp_eval",    "pred_depth.mp4",        "video"),
    ("ChronoDepth",          "chronodepth_depth",     "chronodepth_depth_disp",      "pred_depth.mp4",        "video"),
    ("GEO4D",                "geo4d_depth",           "geo4d_depth_disp",            "pred_depth.mp4",        "video"),
]

TASK_METHODS = {"nvs": NVS_METHODS, "pose": POSE_METHODS,
                "depth": DEPTH_METHODS, "depth_disp": DEPTH_DISP_METHODS}
TASK_TITLES  = {"nvs": "Novel-view synthesis", "pose": "Camera pose",
                "depth": "Depth", "depth_disp": "Depth (disparity space)"}
TASK_HEADERS = {
    "nvs":   ("PSNR &uarr; / LPIPS &darr; / SSIM &uarr;", "PSNR / LPIPS / SSIM",
              nvs,   ("{:.2f}", "{:.3f}", "{:.3f}")),
    "pose":  ("ATE &darr; / RPE_t &darr; / RPE_r &darr;", "ATE / RPE_t / RPE_r",
              pose,  ("{:.3f}", "{:.3f}", "{:.2f}")),
    "depth": ("Abs Rel &darr; / RMSE &darr; / &delta;<1.25 &uarr;", "Abs Rel / RMSE / &delta;<1.25",
              depth, ("{:.3f}", "{:.3f}", "{:.3f}")),
    "depth_disp": (
        "Abs Rel &darr; / RMSE &darr; / &delta;<1.25 &uarr; (disparity-space LAD2; not directly comparable to depth-space numbers)",
...
DATASET_ORDER = [
    ...
    ("dl3dv",          "DL3DV",                                ["nvs", "pose", "depth", "depth_disp"]),
    ("dl3dv_test",     "DL3DV (test split)",                   ["nvs", "pose"]),
    ("tanksandtemples","Tanks & Temples",                      ["nvs", "pose"]),
    ("scannetpp",      "Scannet++",                            ["nvs", "pose", "depth", "depth_disp"]),
    ("vkitti2",        "Virtual KITTI 2 (synthetic, outdoor)", ["nvs", "pose", "depth", "depth_disp"]),
    ("scenenet_depth", "SceneNet (synthetic, indoor)",         ["depth", "depth_disp"]),
    ("spatialvid_nvs", "SpatialVid",                           ["nvs", "pose"]),
    ("agibot_world",   "AgiBotWorld (static head-camera)",     ["nvs", "pose", "depth", "depth_disp"]),
    ("aria",           "Aria (synthetic-grade indoor)",        ["nvs", "pose", "depth", "depth_disp"]),
    ("7scenes",        "7Scenes (zero-shot indoor)",           ["nvs", "pose", "depth", "depth_disp"]),
    ("tum",            "TUM (zero-shot indoor)",               ["nvs", "pose", "depth", "depth_disp"]),
    ("bonn",           "Bonn (zero-shot, depth-only)",         ["depth", "depth_disp"]),
]
...
def get_prompt(dataset, sample_idx):
    ...
    if dataset not in PROMPT_DATASETS:
        return None
    for primary in ("ours_final_pose_depth", "ours_nvs_pose_depth", "ours_final_nvs", "ours_nvs_nvs"):
...
def count_samples(dataset, task):
    ...
    For V4 we have two ours rows (final + nvs); use the max sample count between them."""
    primaries = {
        "nvs":        ["ours_final_nvs",        "ours_nvs_nvs"],
        "pose":       ["ours_final_pose_depth", "ours_nvs_pose_depth"],
        "depth":      ["ours_final_pose_depth", "ours_nvs_pose_depth"],
        "depth_disp": ["ours_final_pose_depth", "ours_nvs_pose_depth"],
    }[task]
    best = 0
    for p in primaries:
        base = EVAL / dataset / p
        if not base.exists(): continue
        n = sum(1 for s in base.iterdir() if s.is_dir() and s.name.startswith("sample_"))
        best = max(best, n)
    return best
...
def build_metric_table(dataset, task):
    ...
    #   pose:  (ATE, RPE_t, RPE_r)        → ATE ↓        (min)
    #   depth: (Abs Rel, RMSE, δ<1.25)    → δ<1.25 ↑    (max)
    PRIMARY_SPEC = {"nvs": (0, "max"), "pose": (0, "min"),
    ...
    best_label = None
    best_val = float("-inf") if primary_dir == "max" else float("inf")
    for label, m in table_rows:
        ...
        if v is None:
            continue
        better = (v > best_val) if primary_dir == "max" else (v < best_val)
        if better:
            best_val = v
            best_label = label
...
<style>
  ...
  /* Pagination nav */
  .ds-nav { position: sticky; top: 0; background: #fff; padding: 12px 0; border-bottom: 1px solid #e5e7eb;
            margin-bottom: 24px; z-index: 100; display: flex; flex-wrap: wrap; gap: 6px; align-items: center; }
  .ds-nav .label { font-size: 12px; color: #6b7280; margin-right: 8px; text-transform: uppercase;
                   letter-spacing: 0.04em; font-weight: 600; }
  .ds-nav button { font-family: inherit; font-size: 13px; padding: 6px 12px;
                   background: #fff; color: #111; border: 1px solid #e5e7eb;
                   border-radius: 999px; cursor: pointer; transition: background 0.15s; }
  .ds-nav button:hover { background: #f3f4f6; }
  .ds-nav button.active { background: #111; color: #fff; border-color: #111; }
  .ds-page[hidden] { display: none; }
  .method-card video, .method-card img { background: #f3f4f6; }
  .method-card .lazy-pending::before {
    content: "loading…"; font-size: 11px; color: #9ca3af;
    display: block; padding: 30px 10px; text-align: center;
  }
</style>
</head>
<body>
...
// Build only carousels that are inside currently-visible (non-hidden) pages.
function initVisible() {
  document.querySelectorAll(".ds-page:not([hidden]) .carousel:not([data-built])").forEach(el => {
    el.setAttribute("data-built", "1");
    buildCarousel(el);
  });
}

// Pagination: show one dataset page at a time (or "all").
function showPage(target) {
  document.querySelectorAll(".ds-page").forEach(p => {
    p.hidden = !(target === "all" || p.dataset.ds === target);
  });
  document.querySelectorAll(".ds-nav button").forEach(b => {
    b.classList.toggle("active", b.dataset.ds === target);
  });
  initVisible();
  // Pause videos on hidden pages so the browser frees decode threads.
  document.querySelectorAll(".ds-page[hidden] video").forEach(v => { try { v.pause(); } catch(_) {} });
}

// Idempotent nav button setup — uses a flag so we only attach listeners once.
function initNav() {
  document.querySelectorAll(".ds-nav button:not([data-nav-bound])").forEach(b => {
    b.setAttribute("data-nav-bound", "1");
    b.addEventListener("click", () => showPage(b.dataset.ds));
  });
}
...
// Run at every reasonable opportunity. Gradio's gr.HTML injects content via
// innerHTML AFTER our scripts execute (which were moved to <head>), so we
// can't rely on DOMContentLoaded alone — a MutationObserver catches the late
// body insertion and re-runs init.
...
def main():
    ...
    # Pagination nav: one button per dataset that has any sample data, plus "All".
    # First dataset is shown by default; the rest are hidden (lazy-built on click).
    rendered_datasets = []
    for ds_key, ds_label, tasks in DATASET_ORDER:
        if any(count_samples(ds_key, t) > 0 for t in tasks):
            rendered_datasets.append((ds_key, ds_label, tasks))

    nav_buttons.append('<span class="label">Dataset:</span>')
    for i, (ds_key, ds_label, _) in enumerate(rendered_datasets):
        active = " active" if i == 0 else ""
        nav_buttons.append(
            f'<button class="ds-btn{active}" data-ds="{ds_key}">{ds_label}</button>'
        )
    nav_buttons.append('<button class="ds-btn" data-ds="all">All (slow)</button>')
    body_parts.append(f'<div class="ds-nav">{" ".join(nav_buttons)}</div>')

    for i, (ds_key, ds_label, tasks) in enumerate(rendered_datasets):
        hidden = "" if i == 0 else " hidden"
        body_parts.append(f'<div class="ds-page" data-ds="{ds_key}"{hidden}>')
